...
PostgreSQL — see the [SSO guide](/guides/sso/)). A single generic OIDC
...
| `OTEL_FLEET_BOOTSTRAP_TOKEN` | edge agent (supervisor) | Per-customer enrollment token, injected into the OpAMP `Authorization` header. See [Edge agents](/guides/edge-agents/). |
