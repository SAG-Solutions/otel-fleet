...
The metrics land in VictoriaMetrics, which speaks the Prometheus query API — so
point **Grafana** at it as a Prometheus datasource (URL
`http://<victoriametrics>:8428`) and import your preferred Kubernetes
dashboards; the receivers emit standard OpenTelemetry `k8s.*` / `system.*` /
`container.*` metrics (dots become `_` after remote-write, e.g.
`k8s_node_cpu_utilization`). For alerting you don't need Alertmanager: create
